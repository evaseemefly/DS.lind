    ...
    public class WxPayData
    {
        ...
        */
            ...
            FromXml(string xml)
        {
            ...
            foreach (var i in m_values.Keys)
            {
                Log.Info(i, m_values[i].ToString());
            }
            try
            {
                ...
                //2015-06-29 错误是没有签名
                //if (m_values.ContainsKey("return_code") && m_values["return_code"] != "SUCCESS")
                //{
                //    return m_values;
                //}
                Log.Info("CheckSign", "签名");
                CheckSign();//验证签名,不通过会抛异常
            }
            catch (WxPayException ex)
            {
                Log.Error("SortedDictionary2", ex.Message);
