...
namespace Lind.DDD.OnlinePay.WeixinJSApi.business
{
    public class JsApiPay
    {
        ...
        /// <summary>
        /// 通过appid和url得到微信颁发的code，然后重定向到用户自己的页面
        /// </summary>
        /// <param name="appid">领域项目的appid</param>
        /// <param name="returnUrl">领域项目的url</param>
        public void GetCodeByAppId(string appid)
        {
            if (!string.IsNullOrEmpty(page.Request.QueryString["code"]))
            {
                ...
                string url = "http://" + page.Request.Url.Host + page.Request.Url.PathAndQuery;
                page.Response.Redirect(url);//跳转到具体领域
            }
            else
            {
                ...
                try
                {
                    Log.Debug("test", "当前域名" + redirect_uri);
                    //触发微信返回code码         
        ...
        */
        ...
        public string GetUnionIdByOpenId(string openId)
        {
            ...
            string url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=" + data.ToUrl() + "&openid=" + openid + "&lang=zh_CN";
	    ...
	    */
         ...
         * @失败时抛异常WxPayException
         */
        public WxPayData GetUnifiedOrderResult(string orderID)
        {
            //统一下单
            WxPayData data = new WxPayData();
