...
namespace PureCat.Message.Spi.IO
{
    public class TcpMessageSender : IMessageSender
    {
        ...
        public void Initialize()
        {
            ...
            PureCat.Util.Logger.Info("Thread(TcpMessageSender-ChannelManagementTask) started.");
            PureCat.Util.Logger.Info("Thread(TcpMessageSender-AsynchronousSendTask) started.");
        }
                ...
                if (_mActive)
                {
                    if (_mActiveChannel == null || !_mActiveChannel.Connected)
                    {
                        if (_mActiveChannel != null)
                            PureCat.Util.Logger.Warn("ChannelManagementTask中，Socket关闭");
        ...
        public void AsynchronousSendTask(object o)
        {
            while (true)
            {
                if (_mActive)
                {
                    ...
                    {
                        if (_mActiveChannel != null && !_mActiveChannel.Connected)
                            PureCat.Util.Logger.Warn("AsynchronousSendTask中，Socket关闭");
                        Thread.Sleep(500);
                    }
                    ...
                    {
                        PureCat.Util.Logger.Error("Error when sending message over TCP socket! Error: {0}", t);
                    }
                }
                else
                {
        ...
        private void SendInternal(IMessageTree tree)
        {
            if (_mLastChannel != null)
            {
                try
                {
                    PureCat.Util.Logger.Warn("SendInternal中，_mLastChannel关闭");
                    _mLastChannel.Close();
                }
                catch
                {
            ...
            {
                PureCat.Util.Logger.Warn("SendInternal中，Socket关闭");
            }
        }
