...
namespace Lind.DDD.OnlinePay
{
    ...
    public abstract class PayBase
    {
        ...
        public void OnlineTo(string orderid, int sourceType, int userID, decimal money, string productname)
        {
            ...
            string body = Utility.EncryptString(string.Format("{0}|{1}|{2}|{3}|{4}"
                ...
                , money
                , sourceType));
            HttpContext.Current.Response.Redirect(GenerateUrl(UseType.Order, body, money, userID, ref exchangeID, ReturnAddress, NotifyAddress, productname));
        }
        /// <summary>
        /// 根据使用方法得到订单商品名称
        /// </summary>
        ...
        protected string GenerateSubject(UseType useType)
        {
            switch (useType)
            {
                case UseType.Recharge:
                    return "{0}(" + SiteName + "充值)";
                case UseType.Order:
                    return "{0}(" + SiteName + "付款)";
                default:
        ...
        /// <returns></returns>
        protected abstract string GenerateUrl(UseType useType, string body, decimal vMoney, int userID, ref string exchangeID, string returnUrl, string notifyUrl, string name);
        #endregion
